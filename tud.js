...
const MainScreen = () => {
  ...
  const fetchData = () => {
    axios
      .get("https://bbef-212-56-211-206.ngrok.io/api/parking-list")
      .then((response) => {
  ...
  const updateOccupiedStatus = (id, isOccupied, token) => {
    ...
    const apiUrl =
      "https://bbef-212-56-211-206.ngrok.io/api/parking-list/" +
      id +
      "/" +
      newStatus;
  ...
  const renderItem = (item, index) => (
    <Row
      ...
      data={[
        item.id.toString(),
        renderOccupiedButton(item.id, item.isOccupied),
      ]}
